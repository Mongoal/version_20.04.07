...
abspath = os.path.realpath('../data/DataSet.h5')
...
for i, x in enumerate(train_indices):
    ...
    plt.savefig('fig/'+'bias to center train ' + x.name + '.png')
...
for i, x in enumerate(test_indices):
    ...
    plt.savefig('fig/'+'bias to center test ' + x.name + '.png')
...
for i, x in enumerate(unknown_indices):
    ...
    plt.savefig('fig/'+'bias to center unknown '+x.name+'.png')
...
for i, x in enumerate(unknown_indices):
    ...
    plt.savefig('fig/'+'distance to train center, unknown '+x.name+' & train '+tx.name+'.png')
...
for x in train_indices:
    for i, h in enumerate(np.random.permutation(len(x))[:3]):
        ...
        plt.savefig('fig/'+'train '+x.name+' '+str(i)+'.png')

for x in unknown_indices:
    for i, h in enumerate(np.random.permutation(len(x))[:3]):
        plt.figure(figsize=(8, 4))
        plt.imshow(emb_unknown[x.indices[h]].reshape(8, 16), cmap=plt.cm.Oranges)
        plt.colorbar()
        plt.title('unknown ' + x.name + ' ' + str(i))
        plt.savefig('fig/' + 'unknown ' + x.name + ' ' + str(i) + '.png')
#
