...
class AutoEncodingConv1dBNModel(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            self.loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.x, logits=self.logits), name='bce')
...
def model(input, is_training=False):
    ...
    net = conv1d_BN(net, input.shape[-1], FILTER_SIZE, is_training, activation='linear',
                           kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
