...
class AutoEncodingConv2dBNModel(BaseModel):
    ...
    def build_model(self):
        ...
        self.decode = tf.sigmoid(self.logits)
...
def model(input, is_training=False):
    ...
    DEPTHS = [32, 32, 64, 128, 256, 128, 64, 32 , 16]
    """
    自编码器-全一维卷积 模型
    ...
    net = conv2d_BN(net, DEPTHS[3], FILTER_SIZE, is_training, stride=STRIDE, name='conv_4',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))

    net = conv2d_BN(net, DEPTHS[4], FILTER_SIZE, is_training, stride=STRIDE, name='conv_4',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))

    flatten = tf.layers.flatten(net)
    fully_conn = tf.layers.dense(flatten, 2048,  kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    net = conv2d_BN_transpose(net, DEPTHS[4], FILTER_SIZE, is_training, stride=STRIDE, name='deconv_1',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    net = conv2d_BN_transpose(net, DEPTHS[5], FILTER_SIZE, is_training, stride=STRIDE, name='deconv_1',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    net = conv2d_BN_transpose(net, DEPTHS[6], FILTER_SIZE, is_training, stride=STRIDE, name='deconv_2',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    net = conv2d_BN_transpose(net, DEPTHS[7], FILTER_SIZE, is_training, stride=STRIDE, name='deconv_3',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    net = conv2d_BN_transpose(net, DEPTHS[8], FILTER_SIZE, is_training, stride=STRIDE, name='deconv_4',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
    net = tf.layers.conv2d(net, input.shape[-1], FILTER_SIZE, padding='same',  name='decode',
                    kernel_initializer=tf.truncated_normal_initializer(stddev=0.1))
